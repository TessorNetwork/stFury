...
while true; do
    N_VALIDATORS_DREDGER=$($DREDGER_MAIN_CMD q tendermint-validator-set | grep -o address | wc -l | tr -dc '0-9')
    echo "DREDGER @ $($DREDGER_MAIN_CMD q tendermint-validator-set | head -n 1 | tr -dc '0-9') | $N_VALIDATORS_DREDGER VALS" >$TEMP_LOGS_DIR/$STATE_LOG
    echo "DREDGER @ $($DREDGER_MAIN_CMD q tendermint-validator-set | head -n 1 | tr -dc '0-9') | $N_VALIDATORS_DREDGER VALS" >$TEMP_LOGS_DIR/$BALANCES_LOG
    ...
    $DREDGER_MAIN_CMD q stakeibc list-host-zone >>$TEMP_LOGS_DIR/$STATE_LOG
    printf '\n%s\n' "LIST-DEPOSIT-RECORDS" >>$TEMP_LOGS_DIR/$STATE_LOG
    $DREDGER_MAIN_CMD q records list-deposit-record  >> $TEMP_LOGS_DIR/$STATE_LOG
    printf '\n%s\n' "LIST-EPOCH-UNBONDING-RECORDS" >>$TEMP_LOGS_DIR/$STATE_LOG
    $DREDGER_MAIN_CMD q records list-epoch-unbonding-record  >> $TEMP_LOGS_DIR/$STATE_LOG
    printf '\n%s\n' "LIST-USER-REDEMPTION-RECORDS" >>$TEMP_LOGS_DIR/$STATE_LOG
    $DREDGER_MAIN_CMD q records list-user-redemption-record >> $TEMP_LOGS_DIR/$STATE_LOG

    printf '\n%s\n' "BALANCES DREDGER" >>$TEMP_LOGS_DIR/$BALANCES_LOG
    $DREDGER_MAIN_CMD q bank balances $(DREDGER_ADDRESS) >>$TEMP_LOGS_DIR/$BALANCES_LOG
