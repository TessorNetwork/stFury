...
set_dred_genesis() {
    ...
    # update params
    jq '(.app_state.epochs.epochs[] | select(.identifier=="day") ).duration = $epochLen' --arg epochLen $DRED_DAY_EPOCH_DURATION $genesis_config > json.tmp && mv json.tmp $genesis_config
    jq '(.app_state.epochs.epochs[] | select(.identifier=="dred_epoch") ).duration = $epochLen' --arg epochLen $DRED_EPOCH_EPOCH_DURATION $genesis_config > json.tmp && mv json.tmp $genesis_config
...
if [ "$CHAIN" == "DREDGER" ]; then 
    ...
    echo "$DRED_ADMIN_MNEMONIC" | $MAIN_CMD keys add $DRED_ADMIN_ACCT --recover --keyring-backend=test >> $KEYS_LOGS 2>&1
    DRED_ADMIN_ADDRESS=$($MAIN_CMD keys show $DRED_ADMIN_ACCT --keyring-backend test -a)
    $MAIN_CMD add-genesis-account ${DRED_ADMIN_ADDRESS} ${ADMIN_TOKENS}${DENOM}
