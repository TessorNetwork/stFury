...
$DRED_MAIN_CMD tx stakeibc register-host-zone \
    $CONNECTION $HOST_DENOM $ADDRESS_PREFIX $IBC_DENOM $CHANNEL 1 \
    --gas 1000000 --from $DRED_ADMIN_ACCT --home $DOCKERNET_HOME/state/dred1 -y | TRIM_TX
sleep 10
...
for (( i=1; i <= $NUM_VALS; i++ )); do
    ...
    $DRED_MAIN_CMD tx stakeibc add-validator $CHAIN_ID ${VAL_PREFIX}${i} $delegate_val 10 $weight \
        --from $DRED_ADMIN_ACCT -y | TRIM_TX
    sleep 10
done

timeout=100
while true; do
    if ! $DRED_MAIN_CMD q stakeibc show-host-zone $CHAIN_ID | grep Account | grep -q null; then
        break
    else
        if [[ "$timeout" == "0" ]]; then 
