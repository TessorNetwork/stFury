...
for chain in ${HOST_CHAINS[@]}; do
    ...
    printf "DREDGER <> $chain - Adding relayer keys..."
    $relayer_exec rly keys restore dredger $RELAYER_DREDGER_ACCT "$mnemonic" >> $relayer_logs 2>&1
    ...
    printf "DREDGER <> $chain - Creating client, connection, and transfer channel..." | tee -a $relayer_logs
    $relayer_exec rly transact link dredger-${chain_name} >> $relayer_logs 2>&1
    echo "Done."
