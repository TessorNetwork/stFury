...
$STRIDE_MAIN_CMD tx bank send val1 dredger1z835j3j65nqr6ng257q0xkkc9gta72gf48txwl 600000udred --from val1 -y
sleep 5
# Fund the airdrop account
$STRIDE_MAIN_CMD tx bank send val1 dredger1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr 1000000000udred --from val1 -y
sleep 5
# Create the airdrop, so that the airdrop account can claim tokens
$STRIDE_MAIN_CMD tx claim create-airdrop dredger 1666792900 40000000 udred --from distributor-test -y
sleep 5
# Set airdrop allocations
$STRIDE_MAIN_CMD tx claim set-airdrop-allocations dredger dredger1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr 1 --from distributor-test -y
sleep 5

# AIRDROP CLAIMS
# Check balances before claims
echo "Initial balance before claim:"
$STRIDE_MAIN_CMD query bank balances dredger1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
...
echo "Staking..."
$STRIDE_MAIN_CMD tx staking delegate dredgervaloper1nnurja9zt97huqvsfuartetyjx63tc5zrj5x9f 100udred --from airdrop-test --gas 400000
sleep 5
