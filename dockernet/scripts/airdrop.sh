...
echo "barrel salmon half click confirm crunch sense defy salute process cart fiscal sport clump weasel render private manage picture spell wreck hill frozen before" | \
    $DRED_MAIN_CMD keys add distributor-test --recover
...
echo "royal auction state december october hip monster hotel south help bulk supreme history give deliver pigeon license gold carpet rabbit raw wool fatigue donate" | \
    $DRED_MAIN_CMD keys add airdrop-test --recover
...
$DRED_MAIN_CMD tx bank send val1 dred1z835j3j65nqr6ng257q0xkkc9gta72gf48txwl 600000udred --from val1 -y
sleep 5
# Fund the airdrop account
$DRED_MAIN_CMD tx bank send val1 dred1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr 1000000000udred --from val1 -y
sleep 5
# Create the airdrop, so that the airdrop account can claim tokens
$DRED_MAIN_CMD tx claim create-airdrop dredger 1666792900 40000000 udred --from distributor-test -y
sleep 5
# Set airdrop allocations
$DRED_MAIN_CMD tx claim set-airdrop-allocations dredger dred1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr 1 --from distributor-test -y
sleep 5

# AIRDROP CLAIMS
# Check balances before claims
echo "Initial balance before claim:"
$DRED_MAIN_CMD query bank balances dred1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
# NOTE: You can claim here using the CLI, or from the frontend!
# Claim 20% of the free tokens
echo "Claiming fee amount..."
$DRED_MAIN_CMD tx claim claim-free-amount --from airdrop-test --gas 400000
sleep 5
echo "Balance after claim:" 
$DRED_MAIN_CMD query bank balances dred1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
# Stake, to claim another 20%
echo "Staking..."
$DRED_MAIN_CMD tx staking delegate dredvaloper1nnurja9zt97huqvsfuartetyjx63tc5zrj5x9f 100udred --from airdrop-test --gas 400000
sleep 5
echo "Balance after stake:" 
$DRED_MAIN_CMD query bank balances dred1nf6v2paty9m22l3ecm7dpakq2c92ueyununayr
# Liquid stake, to claim the final 60% of tokens
echo "Liquid staking..."
$DRED_MAIN_CMD tx stakeibc liquid-stake 1000 uatom --from airdrop-test --gas 400000
sleep 5
