...
# CHAIN PARAMS
BLOCK_TIME='5s'
DRED_DAY_EPOCH_DURATION="120s"
DRED_EPOCH_EPOCH_DURATION="120s"
...
NODE_NAME="dred1"
DRED_VAL_MNEMONIC="close soup mirror crew erode defy knock trigger gather eyebrow tent farm gym gloom base lemon sleep weekend rich forget diagram hurt prize fly"
...
$CMD init DREDGER --chain-id $DRED_CHAIN_ID --overwrite &> /dev/null
...
sed -i -E "s|chain-id = \"\"|chain-id = \"${DRED_CHAIN_ID}\"|g" $client_toml
...
echo "$DRED_VAL_MNEMONIC" | $CMD keys add $DRED_VAL_ACCT --recover --keyring-backend=test >> $KEYS_LOGS 2>&1
VAL_ADDR=$($CMD keys show $DRED_VAL_ACCT --keyring-backend test -a)
...
$CMD gentx $DRED_VAL_ACCT ${STAKE_TOKENS}${DENOM} --chain-id $DRED_CHAIN_ID --keyring-backend test &> /dev/null
...
echo "$DRED_ADMIN_MNEMONIC" | $CMD keys add $DRED_ADMIN_ACCT --recover --keyring-backend=test >> $KEYS_LOGS 2>&1
DRED_ADMIN_ADDRESS=$($CMD keys show $DRED_ADMIN_ACCT --keyring-backend test -a)
$CMD add-genesis-account ${DRED_ADMIN_ADDRESS} ${ADMIN_TOKENS}${DENOM}
...
# update params
jq '(.app_state.epochs.epochs[] | select(.identifier=="day") ).duration = $epochLen' --arg epochLen $DRED_DAY_EPOCH_DURATION $genesis_json > json.tmp && mv json.tmp $genesis_json
jq '(.app_state.epochs.epochs[] | select(.identifier=="dred_epoch") ).duration = $epochLen' --arg epochLen $DRED_EPOCH_EPOCH_DURATION $genesis_json > json.tmp && mv json.tmp $genesis_json
