...
# Start dredger
bash ${SCRIPT_DIR}/init_dredger.sh $STRIDE_CHAIN_ID

$DOCKER_COMPOSE up -d dredger1
$DOCKER_COMPOSE logs -f dredger1 | sed -r -u "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" > $STRIDE_LOGS 2>&1 &
...
$RELAYER_CMD keys restore dredger rly1 "$RELAYER_STRIDE_MNEMONIC" 
