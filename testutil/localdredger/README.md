...
Inspired by LocalOsmosis, LocalDred is a complete Dredger testnet containerized with Docker and orchestrated with a simple docker-compose file. LocalDred comes pre-configured with opinionated, sensible defaults for a standard testing environment.
...
> Note
>
> You can also start LocalDred in detach mode with:
>
...
- Example: `dred tx bank send ls-test2 dred1kwax6g0q2nwny5n43fswexgefedge033t5g95j --keyring-backend test --chain-id localdred`
...
## 2. LocalDred - With Mainnet State
...
if mainnet is on v8.0.0 and you try to do this on a v9.0.0 tag or on main, you will run into an error when initializing the genesis. What you can do though is run localdred on the mainnet version, then go through the upgrade process to ensure the upgrade with mainnet state goes smoothly.
...
### Use the state export in LocalDred
...
```sh
dred tx bank send val dred1qym804u6sa2gvxedfy96c0v9jc0ww7593uechw 10000000udred --chain-id localdred --keyring-backend test
```
...
LocalDred is pre-configured with one validator and 10 accounts with dredger balances.

| Account   | Address                                                                                                | Mnemonic                                                                                                                                                                   |
| --------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| val    | `dred1wal8dgs7whmykpdaz0chan2f54ynythkz0cazc`<br/>`dredvaloper1wal8dgs7whmykpdaz0chan2f54ynythkp6upwa` | `deer gaze swear marine one perfect hero twice turkey symbol mushroom hub escape accident prevent rifle horse arena secret endless panel equal rely payment`                    |
