...
then
    ...
    echo "DRED_HOME: $DRED_HOME"

    echo $MNEMONIC | dred init localdred -o --chain-id=$CHAIN_ID --home $DRED_HOME
    ...
    VALIDATOR_PUBKEY_JSON=$(dred tendermint show-validator --home $DRED_HOME)
    VALIDATOR_PUBKEY=$(echo $VALIDATOR_PUBKEY_JSON | dasel -r json '.key' --plain)
    VALIDATOR_HEX_ADDRESS=$(dred debug pubkey $VALIDATOR_PUBKEY_JSON 2>&1 --home $DRED_HOME | grep Address | cut -d " " -f 2)
    VALIDATOR_ACCOUNT_ADDRESS=$(dred debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $DRED_HOME | grep Acc | cut -d " " -f 3)
    VALIDATOR_OPERATOR_ADDRESS=$(dred debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $DRED_HOME | grep Val | cut -d " " -f 3)
    VALIDATOR_CONSENSUS_ADDRESS=$(dred tendermint show-address --home $DRED_HOME)
...
dred start --home $DRED_HOME --x-crisis-skip-assert-invariants
