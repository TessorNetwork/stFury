...
[Twitter](https://twitter.com/dred_zone) | [Discord](http://dredger.zone/discord) | [Website](https://dredger.zone/)
## What is Dredger?


Dredger is a blockchain ("zone") that provides liquidity for staked assets. Using Dredger, you can earn *both* staking *and* DeFi yields across the Cosmos IBC ecosystem. Read our ["Meet Dredger" blog post](https://dredger.zone/blog/meet-dredger) to learn more about why we built Dredger. 

**Dredger** is built using Cosmos SDK and Tendermint. Dredger allows users to liquid stake any IBC-compatible cosmos SDK native appchain token. Under the hood, Dredger leverages the [Inter-Blockchain Communication protocol](https://ibc.cosmos.network/), [Interchain Accounts](https://blog.cosmos.network/interchain-accounts-take-cosmos-interoperability-to-the-next-level-39c9a8aad4ad) and [Interchain Queries](https://github.com/schnetzlerjoe/interchain-query-spec).
...
If you want to setup your node for the Dredger mainnet, find the relevant files and instructions [here](https://github.com/TessorNetwork/mainnet/blob/main/mainnet/README.md)

## Getting Started as a Developer
### Developing on Dredger

Developers who wish to develop on Dredger can easily spin up 3 Dredger nodes, 3 Gaia nodes, 1 Hermes relayer and 1 interchain-queries relayer. The Gaia nodes simulate the Cosmos Hub (Gaia) zone in local development node, and the relayers allow Dredger zone to interact with that instance of Gaia.

The fastest way to develop on Dredger is local development mode.
...
1. `s` This will re-build the Dredger binary (default)
...
- Rebuild the Dredger and Gaia binaries
- Start 1 Dredger and 1 Gaia node in the docker
- Start Relayers
...
To test the chain with a mnemonic that has tokens (e.g. sending Dredger transactions), you can use the VAL_MNEMONIC_1, which is
```
...
## Dredger's Technical Architecture

Users stake their tokens on Dredger from any Cosmos chain. Rewards accumulate in real time. No minimum. They will receive staked tokens immediately when they liquid stake. These staked tokens can be freely traded, and can be redeemed with Dredger at any time to receive your original tokens plus staking rewards.

<img src="https://drive.google.com/uc?id=1CjEkovV3tHspZxlgP8b_th5NPb78T9nB" width="920">

On the backend, Dredger permissionly stakes these tokens on the host chain and compounds user rewards. Dredger lets users use your staked assets to compound their yields. Continue to earn staking yield, and earn additional yield by lending, LPing, and more. They can set their own risk tolerance in Cosmos DeFi.  

<img src="https://drive.google.com/uc?id=11kJZE93BdhNjkaNig3DGYnTnuSNClP8Y" width="900">

Users can always redeem from Dredger. When they select "redeem" on the Dredger website, Dredger will initiate unbonding on the host zone. Once the unbonding period elapses, the users will receive native tokens in their wallets. 
