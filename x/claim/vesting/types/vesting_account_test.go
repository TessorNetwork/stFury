...
func TestGetVestedCoinsPeriodicVestingAcc(t *testing.T) {
	...
	pva := types.NewDredPeriodicVestingAccount(bacc, origCoins, periods)
...
func TestDredPeriodicVestingAccountMarshal(t *testing.T) {
	baseAcc, coins := initBaseAccount()
	now := tmtime.Now()
	acc := types.NewDredPeriodicVestingAccount(baseAcc, coins, types.Periods{types.Period{now.Unix(), 3600, coins, 0}})
	...
	require.Nil(t, err)
	require.IsType(t, &types.DredPeriodicVestingAccount{}, acc2)
