...
func TestGetVestedCoinsPeriodicVestingAcc(t *testing.T) {
	...
	pva := types.NewDredgerPeriodicVestingAccount(bacc, origCoins, periods)
...
func TestDredgerPeriodicVestingAccountMarshal(t *testing.T) {
	baseAcc, coins := initBaseAccount()
	now := tmtime.Now()
	acc := types.NewDredgerPeriodicVestingAccount(baseAcc, coins, types.Periods{types.Period{now.Unix(), 3600, coins, 0}})
	...
	require.Nil(t, err)
	require.IsType(t, &types.DredgerPeriodicVestingAccount{}, acc2)
