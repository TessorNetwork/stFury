...
func (suite *KeeperTestSuite) TestSetAirdropAllocationsForMultiAirdrops() {
	...
	totalWeightDredger, err := suite.app.ClaimKeeper.GetTotalWeight(suite.ctx, types.DefaultAirdropIdentifier)
	suite.Require().NoError(err)
	suite.Require().Equal(totalWeightDredger, sdk.NewDecWithPrec(90, 2))
