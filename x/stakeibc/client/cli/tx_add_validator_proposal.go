...
`, version.AppName),
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			dredgerDenom, err := sdk.GetBaseDenom()
			if err != nil {
				return err
			}

			if len(deposit) != 1 || deposit.GetDenomByIndex(0) != dredgerDenom {
				return sdkerrors.Wrapf(sdkerrors.ErrInvalidCoins, "Deposit token denom must be %s", dredgerDenom)
			}
