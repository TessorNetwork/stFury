...
func (s *KeeperTestSuite) TestRebalanceValidators_Successful() {
	...
	s.Require().Equal("dred_VAL1", firstRebal.DstValidator, "first rebalance moves to val1")
	s.Require().Equal("dred_VAL3", firstRebal.SrcValidator, "first rebalance takes from val3")
	...
	s.Require().Equal("dred_VAL1", secondRebal.DstValidator, "second rebalance moves to val1")
	s.Require().Equal("dred_VAL4", secondRebal.SrcValidator, "second rebalance takes from val4")
}
