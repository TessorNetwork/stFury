...
func (k Keeper) GetDredEpochElapsedShare(ctx sdk.Context) (sdk.Dec, error) {
	// Get the current dredger epoch
	epochTracker, found := k.GetEpochTracker(ctx, epochstypes.DRED_EPOCH)
	if !found {
		errMsg := fmt.Sprintf("Failed to get epoch tracker for %s", epochstypes.DRED_EPOCH)
		k.Logger(ctx).Error(errMsg)
...
func (k Keeper) IsWithinBufferWindow(ctx sdk.Context) (bool, error) {
	elapsedShareOfEpoch, err := k.GetDredEpochElapsedShare(ctx)
	if err != nil {
		return false, err
	}
