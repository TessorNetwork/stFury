...
type QueryValidatorExchangeRateTestCase struct {
	...
	dredEpochTracker types.EpochTracker
...
func (s *KeeperTestSuite) SetupQueryValidatorExchangeRate() QueryValidatorExchangeRateTestCase {
	...
	s.App.StakeibcKeeper.SetEpochTracker(s.Ctx, dredEpochTracker)
	...
	return QueryValidatorExchangeRateTestCase{
		...
		hostZone:           hostZone,
		dredEpochTracker: dredEpochTracker,
...
func (s *KeeperTestSuite) TestQueryValidatorExchangeRate_BeforeBufferWindow() {
	...
	// set the time to be 50% through the dred_epoch
	dredEpochTracker := tc.dredEpochTracker
	dredEpochTracker.NextEpochStartTime = uint64(s.Coordinator.CurrentTime.UnixNano() + int64(dredEpochTracker.Duration)/2) // 50% through the epoch
	s.App.StakeibcKeeper.SetEpochTracker(s.Ctx, dredEpochTracker)
