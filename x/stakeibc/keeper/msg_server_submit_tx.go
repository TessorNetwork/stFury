...
func (k Keeper) DelegateOnHost(ctx sdk.Context, hostZone types.HostZone, amt sdk.Coin, depositRecord recordstypes.DepositRecord) error {
	...
	_, err = k.SubmitTxsDredEpoch(ctx, connectionId, msgs, *delegationAccount, ICACallbackID_Delegate, marshalledCallbackArgs)
	if err != nil {
...
func (k Keeper) SetWithdrawalAddressOnHost(ctx sdk.Context, hostZone types.HostZone) error {
	...
	_, err = k.SubmitTxsDredEpoch(ctx, connectionId, msgs, *delegationAccount, "", nil)
	if err != nil {
...
func (k Keeper) UpdateWithdrawalBalance(ctx sdk.Context, hostZone types.HostZone) error {
	...
	withdrawalAccount := hostZone.WithdrawalAccount
	if withdrawalAccount == nil || withdrawalAccount.Address == "" {
		return sdkerrors.Wrapf(types.ErrICAAccountNotFound, "no withdrawal account found for %s", hostZone.ChainId)
	}
	...
	ttl, err := k.GetICATimeoutNanos(ctx, epochstypes.DRED_EPOCH)
	if err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidRequest,
			"Failed to get ICA timeout nanos for epochType %s using param, error: %s", epochstypes.DRED_EPOCH, err.Error())
	}
...
func (k Keeper) SubmitTxsDredEpoch(
	ctx sdk.Context,
	connectionId string,
	msgs []sdk.Msg,
	account types.ICAAccount,
	...
	sequence, err := k.SubmitTxsEpoch(ctx, connectionId, msgs, account, epochstypes.DRED_EPOCH, callbackId, callbackArgs)
	if err != nil {
		return 0, err
	}
	return sequence, nil
...
func (k Keeper) QueryValidatorExchangeRate(ctx sdk.Context, msg *types.MsgUpdateValidatorSharesExchRate) (*types.MsgUpdateValidatorSharesExchRateResponse, error) {
	...
	if !found {
		return nil, sdkerrors.Wrapf(types.ErrInvalidHostZone, "Host zone not found (%s)", msg.ChainId)
	}
	...
	ttl, err := k.GetStartTimeNextEpoch(ctx, epochstypes.DRED_EPOCH)
	if err != nil {
