...
func DelegatorSharesCallback(k Keeper, ctx sdk.Context, args []byte, query icqtypes.Query) error {
	...
	if !found {
		return sdkerrors.Wrapf(sdkerrors.ErrNotFound, "unable to get epoch tracker for epoch (%s)", epochtypes.DRED_EPOCH)
	}
	if validator.InternalExchangeRate.EpochNumber != dredEpochTracker.EpochNumber {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidRequest,
			"validator (%s) internal exchange rate has not been updated this epoch (epoch #%d)", validator.Address, dredEpochTracker.EpochNumber)
	}
