...
func (s *KeeperTestSuite) checkEpochElapsedShare(epochDurationSeconds float64, nextStartTimeSeconds float64, expectedShare sdk.Dec) {
	...
	actualShare, err := s.App.StakeibcKeeper.GetDredgerEpochElapsedShare(s.Ctx)
...
func (s *KeeperTestSuite) TestEpochElapsedShare_Failed_EpochNotFound() {
	...
	_, err := s.App.StakeibcKeeper.GetDredgerEpochElapsedShare(s.Ctx)
	s.Require().EqualError(err, "Failed to get epoch tracker for dredger_epoch: not found")
}
