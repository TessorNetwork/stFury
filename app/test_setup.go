...
// Initializes a new DredApp without IBC functionality
func InitDredTestApp(initChain bool) *DredApp {
	db := dbm.NewMemDB()
	app := NewDredApp(
		log.NewNopLogger(),
		db,
...
func GenesisStateWithValSet(app *DredApp) GenesisState {
	privVal := mock.NewPV()
...
func InitDredIBCTestingApp() (ibctesting.TestingApp, map[string]json.RawMessage) {
	app := InitDredTestApp(false)
